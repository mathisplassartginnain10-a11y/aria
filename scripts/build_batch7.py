"""Génère le batch 7 — double le catalogue (~13k alias open/ouvrir + nouveaux sites)."""
...
def main() -> None:
    ...
    start = src.index("def _merged_before_batch7()")
    end = src.index("\ndef main()")
    body = src[start:end].rstrip()
    out = Path(__file__).resolve().parent / "aliases_batch7.py"
    module = (
    ...
    prior = _merged_before_batch7()
    print(f"prior entries: {len(prior)}")
    print(f"batch7 entries: {len(batch)}")
    print(f"total after merge: {len(prior) + len(batch)}")
    print(f"wrote {out}")
